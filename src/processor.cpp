...
void Chip8_Processor::Execute(Memory ram) {
        // Fetch instruction
        unsigned char instruction[2] = {ram.Fetch_Memory(pc), ram.Fetch_Memory(pc + 1)};
        pc += 2;

        // Setting the instruction arguments
        u_int16_t opcode = (instruction[0] << 8) | instruction[1];
        u_int16_t nnn = opcode & 0x0FFF;
        u_int16_t nn = opcode & 0x00FF;
        u_int16_t n = opcode & 0x000F;
        u_int16_t x = (opcode & 0x0F00) >> 8;
        u_int16_t y = (opcode & 0x00F0) >> 4;
